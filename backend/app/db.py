...
def init_db() -> None:
    ...
    with get_connection() as conn:
        with conn.cursor() as cur:
            ...
            # Canvas LTI 1.3 tables share names and shape with ClubALL's 002_lti.sql.
            cur.execute(
                """
                CREATE TABLE IF NOT EXISTS lti_identities_platform (
            ...
            cur.execute(
                """
                CREATE TABLE IF NOT EXISTS lti_course_links_platform (
                    issuer TEXT NOT NULL,
            ...
            cur.execute(
                """
                CREATE TABLE IF NOT EXISTS lti_launch_states_platform (
                    state_hash TEXT PRIMARY KEY,
                    nonce TEXT NOT NULL,
                    issuer TEXT NOT NULL,
            ...
            cur.execute(
                """
                CREATE TABLE IF NOT EXISTS lti_login_codes_platform (
            ...
            cur.execute(
                """
                CREATE INDEX IF NOT EXISTS lti_launch_states_expiry
...
def _user_profile(row: tuple[object, ...] | None) -> dict[str, object] | None:
    ...
    canvas_connected = bool(row[9]) if len(row) > 9 else False
...
USER_PROFILE_COLUMNS = """
    ...
    EXISTS (
        SELECT 1 FROM lti_identities_platform lti
        WHERE lti.user_id = users_platform.id
    )
"""
