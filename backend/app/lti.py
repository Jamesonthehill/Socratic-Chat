...
Ported from ClubALL's platform_app/lti.py for standalone Socratic-Chat hosting.
"""
...
def _connection():
    if not db.is_enabled():
        raise HTTPException(status_code=503, detail="Canvas LTI requires DATABASE_URL.")
    db.init_db()
...
def canvas_configuration():
    ...
    return {
        "title": "Socratic-Chat",
        "description": "Course-grounded Socratic tutoring from your course materials.",
...
async def _start_login(request: Request) -> RedirectResponse:
    ...
    return RedirectResponse(
        f"{settings.LTI_AUTHORIZATION_URL}?{urlencode(query)}",
        status_code=302,
    )
    ...
    with _connection() as conn:
        ...
        if not course_id:
            if role != "instructor":
                raise HTTPException(
                    status_code=409,
                    detail="An instructor must launch Socratic-Chat from this Canvas course before students can enter.",
                )
            course_id = str(uuid4())
...
async def lti_launch(request: Request):
    ...
    # The frontend may be hosted separately (GitHub Pages), so return to FRONTEND_URL.
    separator = "&" if "?" in settings.FRONTEND_URL else "?"
    query = urlencode({"lti": "verified", "code": code, "course": course_id})
    return RedirectResponse(f"{settings.FRONTEND_URL}{separator}{query}", status_code=303)
