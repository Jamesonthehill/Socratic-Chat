...
const CONFIG = window.SOCRATIC_CONFIG || {};
const API_BASE_URL = (CONFIG.API_BASE_URL || "").replace(/\/+$/, "");
export const SOCRATIC_URL = CONFIG.SOCRATIC_URL || "/";
export const PLATFORM_URL = CONFIG.PLATFORM_URL || "/platform/";
...
export async function api(path, options = {}) {
  ...
  const res = await fetch(`${API_BASE_URL}/api${path}`, { ...options, headers, body });
  if (res.status === 401) {
    logout();
